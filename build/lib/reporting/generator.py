...
def sentiment_distribution(analysis_clusters: list[dict]) -> dict[str, int]:
    ...
    for cluster in analysis_clusters:
        # Prefer explicit counts. Newer analysis outputs keep
        # sentiment_distribution as ratios for readability.
        counts = cluster.get("sentiment_counts") or _distribution_to_counts(cluster)
        if counts:
            for sentiment, count in counts.items():
                counter[sentiment] += int(count)
        else:
            sentiment = str(cluster.get("dominant_sentiment") or cluster.get("sentiment", "neutral"))
            size = int(cluster.get("size", 1))
            counter[sentiment] += size
    return dict(counter)


def _distribution_to_counts(cluster: dict) -> dict[str, int]:
    """Convert legacy counts or new ratio distributions into counts."""
    dist = cluster.get("sentiment_distribution") or {}
    if not dist:
        return {}

    values = list(dist.values())
    looks_like_ratio = values and all(isinstance(value, float) for value in values)
    if looks_like_ratio:
        size = int(cluster.get("size", 1))
        return {
            sentiment: int(round(float(dist.get(sentiment, 0.0)) * size))
            for sentiment in SENTIMENT_VALUES
        }

    return {
        sentiment: int(dist.get(sentiment, 0))
        for sentiment in SENTIMENT_VALUES
    }
...
def build_cluster_rows(clusters_data: dict, analysis_data: dict) -> list[dict]:
    ...
    for cluster in clusters_data.get("clusters", []):
        ...
        sentiment_counts = (
            summary.get("sentiment_counts")
            or cluster.get("sentiment_counts")
            or _distribution_to_counts(summary)
            or _distribution_to_counts(cluster)
        )
        ...
        rows.append(
            {
                ...
                "sentiment": summary.get("dominant_sentiment") or summary.get("sentiment", "neutral"),
...
def generate_clusters_csv(rows: list[dict], output_path: Path) -> None:
    with output_path.open("w", encoding="utf-8", newline="") as file:
        ...
        for row in rows:
            row_copy = dict(row)
            dist = row_copy.pop("sentiment_counts", {}) or row_copy.pop("sentiment_distribution", {})
            row_copy.pop("sentiment_distribution", None)
...
def generate_markdown(rows: list[dict], sentiment_counts: dict[str, int], strategies: str = "") -> str:
    ...
    for row in rows:
        dist = row.get("sentiment_counts") or row.get("sentiment_distribution", {})
