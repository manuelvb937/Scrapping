...
def main() -> None:
    ...
    if args.show_config:
        ...
        print(f"  SENTIMENT_METHOD: {settings.sentiment_method}")
        print(f"  SENTIMENT_MODEL_NAME: {settings.sentiment_model_name}")
        print(f"  SENTIMENT_CONFIDENCE_THRESHOLD: {settings.sentiment_confidence_threshold}")
        print(f"  ENABLE_LLM_SENTIMENT_FALLBACK: {settings.enable_llm_sentiment_fallback}")
        print(f"  TOPIC_LABELING_MODEL: {settings.topic_labeling_model}")
        return
